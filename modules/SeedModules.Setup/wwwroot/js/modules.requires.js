define('SeedModules.Setup/modules/controllers/form', [
    'require',
    'exports',
    'SeedModules.Setup/modules/module'
    ...
    var Controller = function () {
        function Controller($scope, $modal, $location, $window, popupService, requestService) {
            ...
            requestService.url('/api/setup').get().result.then(function (result) {
                $scope.data.Name = result.name ? result.name : $scope.data.Name;
                $scope.data.TablePrefix = result.tablePrefix ? result.tablePrefix : $scope.data.TablePrefix;
        ...
        Controller.prototype.install = function () {
            ...
            case 'SqlConnection':
                this.$scope.data.ConnectionString = 'Data Source=' + this.$scope.mssql.Server + ';Initial Catalog=' + this.$scope.mssql.Database + ';User ID=' + this.$scope.mssql.Username + ';Password=' + this.$scope.mssql.Password + ';';
                break;
            case 'MySql': {
                    this.$scope.data.ConnectionString = 'server=' + this.$scope.mysql.Server + ';database=' + this.$scope.mysql.Database + ';uid=' + this.$scope.mysql.Username + ';pwd=' + this.$scope.mysql.Password + ';';
                    if (this.$scope.mysql.Port && this.$scope.mysql.Port !== '') {
                        this.$scope.data.ConnectionString = this.$scope.data.ConnectionString + 'port=' + this.$scope.mysql.Port + ';';
                    }
                    if (!this.$scope.mysql.Ssl) {
                        this.$scope.data.ConnectionString = this.$scope.data.ConnectionString + 'SslMode=none;';
                    }
                    break;
                }
            default:
...
define('SeedModules.Setup/modules/requires', [
    'require',
    'exports',
    'SeedModules.Setup/modules/controllers/form'
