...
namespace Seed.Mvc.FileProviders
{
    public static class FileProviderExtensions
    {
            ...
            bool inDepth = true)
        {
            ...
            if (!inViewsFolder && viewsFolder != null && inDepth)
            {
                ...
                {
                    foreach (var filePath in GetViewFilePaths(fileProvider, string.Format("{0}/{1}", subPath, viewsFolderInfo.Name), plugins, viewsFolder, inViewsFolder: true))
                    {
            ...
            foreach (var content in contents)
            {
                ...
                {
                    foreach (var filePath in GetViewFilePaths(fileProvider, string.Format("{0}/{1}", subPath, content.Name), plugins, viewsFolder, inViewsFolder))
                    {
                        yield return filePath;
                    }
                }
                ...
                {
                    if (plugins.Contains(Path.GetExtension(content.Name)))
                    {
